...
const mapApiDataToMenuItems = (apiData) => {
  ...
  const breakfastCategories = apiData.filter(
    (category) => category.category?.toLowerCase() === "breakfast"
  );
...
export default function Breakfast({ navigation }) {
  const { cartItems, addToCart, removeFromCart, getQty } =
    ...
    <View style={{ flex: 1, backgroundColor: "#eaf7e9" }}>
      ...
      <View style={styles.breakfastheader}>
        <Image
          ...
          style={styles.breakfastheaderImage}
        />
        <View style={styles.breakfastheaderOverlay}>
          <Text style={styles.breakfastheaderTitle}>Breakfast Menu</Text>
          <Text style={styles.breakfastheaderSubtitle}>Start your day fresh!</Text>
        </View>
      </View>
                    ...
                    <View key={food.id || idx} style={styles.subCard}>
                      <View style={styles.subImageBackground}>
                        ...
                        >
                          <Ionicons
                            name={
                              isFavourite(food.id || `${item.name}-${food.name}`)
                                ? "heart"
                                : "heart-outline"
                            }
                            size={16}
...
const styles = StyleSheet.create({
  ...
  breakfastheader: { height: responsive(220), width: "100%" },
  breakfastheaderImage: { width: "100%", height: "100%" },
  breakfastheaderOverlay: {
    position: "absolute",
    top: 0,
    left: 0,
    right: 0,
  ...
  breakfastheaderTitle: { fontSize: responsive(40), fontWeight: "bold", color: "#fff" },
  breakfastheaderSubtitle: {
