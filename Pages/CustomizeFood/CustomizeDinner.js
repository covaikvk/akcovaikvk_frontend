...
import CustomizeAdd3 from "../../Components/CustomizeAdd/CustomizeAdd3";
...
const mapApiDataToMenuItems = (apiData) => {
  ...
  const dinnerCategories = apiData.filter(
    (category) => category.category && category.category.toLowerCase() === "dinner"
  );
...
export default function CustomizeDinner({ navigation, route }) {
  ...
  useEffect(() => {
    const fetchMenuData = async () => {
      try {
        ...
        console.error("Failed to fetch dinner menu:", error);
  ...
  if (isLoading) {
    return (
      <View style={[styles.loadingContainer, { backgroundColor: "#eaf7e9" }]}>
        ...
        <Text style={{ marginTop: responsive(10, 14) }}>Loading Dinner Menu...</Text>
      </View>
    );
  }
  ...
  if (menuItems.length === 0) {
    return (
      <View style={{ flex: 1, backgroundColor: "#eaf7e9" }}>
        <Menuheader />
        <View style={styles.customizedinnerheader}>
          <Image source={require("../../assets/dinner.png")} style={styles.customizedinnerheaderImage} />
          <View style={styles.customizedinnerheaderOverlay}>
            <Text style={styles.customizedinnerheaderTitle}>Dinner Menu</Text>
            <Text style={styles.customizedinnerheaderSubtitle}>Delicious evening meals!</Text>
          </View>
        </View>
        <View style={styles.loadingContainer}>
          <Text style={{ fontSize: responsive(18, 22), color: "#555" }}>No Dinner items found.</Text>
        </View>
        <Footer />
      </View>
    );
...
const styles = StyleSheet.create({
  ...
  customizedinnerheader: { height: responsive(200, 280), width: "100%" },
  customizedinnerheaderImage: { width: "100%", height: "100%" },
  customizedinnerheaderOverlay: { position: "absolute", top: 0, left: 0, right: 0, height: "100%", backgroundColor: "rgba(0,0,0,0.4)", justifyContent: "center", alignItems: "center" },
  customizedinnerheaderTitle: { fontSize: responsive(40, 60), fontWeight: "bold", color: "#fff" },
  customizedinnerheaderSubtitle: { fontSize: responsive(20, 28), color: "#fff", marginTop: responsive(6, 10) },
